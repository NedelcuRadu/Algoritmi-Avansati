...
#Fac graphs pentru valoarea maxima si medie
x = np.array(range(etape))
plt.rcParams["figure.figsize"] = [18, 7]
plt.subplot(1, 2, 1)
plt.plot(x, maxvalues, 'bo', color='green', marker='o', markersize=8, linestyle='--', linewidth=2)
plt.xlabel('Numarul generatiei')
plt.ylabel('Valoare')
plt.title('Evolutia gasirii maximului')
m = np.polyfit(x, maxvalues, 1)
plt.plot(x, m[0] * x + m[1])
plt.grid()
plt.subplot(1, 2, 2)
plt.plot(x, medievalues, 'bo', color='green', marker='o', markersize=8, linestyle='--', linewidth=2)
plt.xlabel('Numarul generatiei')
plt.ylabel('Valoare')
plt.title('Evolutia mediei')
m = np.polyfit(x, medievalues, 1)
plt.grid()
plt.plot(x, m[0] * x + m[1])
plt.show()
